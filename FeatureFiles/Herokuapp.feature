...
Scenario: Scenario_06_GET_Verify the newly updated Book details
	When I submit "GET" request to the "/booking/{bookingid}" endpoint
	Then I should see "200" as status code
	Then I should see "OK" as status line
...
Scenario: Scenario_07_PATCH_Partially update newly created Book details
	When I add following Header in the request
	|Header | Value|
	|Cookie | token={token}|
	And I add following body in request
	| type | Body |
	|Json  | {"firstname" : "Naved1","lastname" : "Ali1","totalprice" : 222,"depositpaid" : false}  |
	And I submit "PATCH" request to the "/booking/{bookingid}" endpoint
	Then I should see "200" as status code
	Then I should see "OK" as status line

Scenario: Scenario_08_GET_Verify the newly partially updated Book details
	When I submit "GET" request to the "/booking/{bookingid}" endpoint
	Then I should see "200" as status code
	Then I should see "OK" as status line
	Then I verify attribute "firstname" has "Naved1" value in response
	Then I verify attribute "lastname" has "Ali1" value in response
	Then I verify attribute "totalprice" has "222" value in response
	Then I verify attribute "depositpaid" has "False" value in response

Scenario: Scenario_09_DELETE_Delete newly created Book details
	When I add following Header in the request
	|Header | Value|
	|Cookie | token={token}|
	And I submit "DELETE" request to the "/booking/{bookingid}" endpoint
	Then I should see "201" as status code
	Then I should see "Created" as status line